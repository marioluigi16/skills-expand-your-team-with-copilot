document.addEventListener("DOMContentLoaded", () => {
  ...
  // Only set up dark mode if the toggle exists
  ...
  function renderActivityCard(name, details) {
    ...
    activityCard.innerHTML = `
      ...
      <div class="social-share">
        ...
        <div class="share-buttons">
          ...
          <button class="share-btn share-linkedin" data-activity="${name}" title="Share on LinkedIn" aria-label="Share on LinkedIn">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    ...
    shareButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        shareActivity(activityName, details, button.classList);
      });
    });
  ...
  function shareActivity(activityName, details, buttonClasses) {
    // Create activity-specific URL with hash fragment
    const activityUrl = `${window.location.origin}${window.location.pathname}#activity-${encodeURIComponent(activityName)}`;
    const activityText = `Check out this activity at Mergington High School: ${activityName} - ${details.description}`;
    const encodedText = encodeURIComponent(activityText);
    const encodedUrl = encodeURIComponent(activityUrl);
    
    let shareUrl = "";
    let isMailto = false;
    
    if (buttonClasses.contains("share-twitter")) {
      shareUrl = `https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedUrl}`;
    } else if (buttonClasses.contains("share-facebook")) {
      shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`;
    } else if (buttonClasses.contains("share-linkedin")) {
      shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`;
    } else if (buttonClasses.contains("share-email")) {
    ...
    if (shareUrl) {
      if (isMailto) {
        // For mailto, just navigate directly without opening a popup
